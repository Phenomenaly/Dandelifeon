...
public:
    static void spawnTshape(Bitboard& board, int x, int y) {
    ...
    static void spawnTshapeSymmetric(Bitboard& board, int y, bool pointingUp) {
        int x = 11;
        if (pointingUp) {
            board.setCell(x, y, true);
            board.setCell(x + 1, y, true);
    ...
    static double getFigureDistance(const Bitboard& cells) {
    ...
    static void translate(const Bitboard& src, Bitboard& dest, int dx, int dy) {
        dest.clear();
        for (int y = 1; y <= 25; ++y) {
            int target_y = y + dy;
    ...
    static void mirrorHorizontal(const Bitboard& src, Bitboard& dest) {
        dest.clear();
        for (int y = 1; y <= 25; ++y) {
    ...
    static void mirrorVertical(const Bitboard& src, Bitboard& dest) {
        dest.clear();
        for (int y = 1; y <= 25; ++y) {
    ...
    static void rotate90(const Bitboard& src, Bitboard& dest) {
        dest.clear();
        for (int y = 1; y <= 25; ++y) {
            uint32_t row = src.data[y];
            if (!row) continue;
    ...
    static void mutateWallsOnFootprint(Bitboard& obstacles, const Bitboard& footstep, const Bitboard& startCells, int numMutations) {
    ...
    static void mutateWallsSymmetric(Bitboard& obstacles, const Bitboard& footstep, int numMutations) {
        std::vector<std::pair<int, int>> activeCoords;
